...
def rnn_cell_backward(da_next, cache):
    ...
    dtanh = np.multiply(1 - np.power(a_next, 2), da_next)
...
$$d\tilde c^{\langle t \rangle} = \left(dc_{next}*\Gamma_u^{\langle t \rangle}+ \Gamma_o^{\langle t \rangle} * (1-\tanh(c_{next})^2) * \Gamma_u^{\langle t \rangle} * da_{next} * \tilde c^{\langle t \rangle}\right) * (1-\tilde c^2) \tag{8}$$

$$d\Gamma_u^{\langle t \rangle} = \left(dc_{next}*\tilde c^{\langle t \rangle} + \Gamma_o^{\langle t \rangle} * (1-\tanh(c_{next})^2) * \tilde c^{\langle t \rangle} * da_{next}*\Gamma_u^{\langle t \rangle}\right)*(1-\Gamma_u^{\langle t \rangle})\tag{9}$$

$$d\Gamma_f^{\langle t \rangle} = \left(dc_{next}* c_{prev} + \Gamma_o^{\langle t \rangle} * (1-\tanh(c_{next})^2) * c_{prev} * da_{next}*\Gamma_f^{\langle t \rangle}\right)*(1-\Gamma_f^{\langle t \rangle})\tag{10}$$
